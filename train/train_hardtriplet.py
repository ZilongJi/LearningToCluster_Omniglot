"""
python ./train/train_hardtriplet.py --hard_mining True

testing in 5-way 1-shot mode: 
    python ./train/train_hardtriplet.py --only_test True --test_way 5 --test_shot 1
...
def train(args, model, device, optimizer, tri_loss, exp_dir):
    ...
    for iter_n in range(args.iteration): 
        ...
        for ep in range(args.epochs):
            ...
            for data, target in tqdm(train_loader):    
                ...
                totalloss_meter.update(totalloss.item())
                
            #Epoch log
            time_log = 'Ep {}, {:.2f}s'.format(ep, time.time() - ep_st, )  
            
            loss_log = (', triloss {:.4f}, total loss {:.4f}'.format(triloss_meter.avg, totalloss_meter.avg))                         
    ...
    with open('./doc/new_sets/new_dataset.pickle', 'wb') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ims_per_id', type=int, default=6, 
                        help="ims_per_id must >1 when using triplet loss")
    ...
    parser.add_argument('--margin', type=float, default=1.0, help="margin of the triplet loss, default: 0.3")
