"""
python ./train/train_proto_hardtriplet_supportonly.py

testing in 5-way 1-shot mode: 
    python ./train/train_proto_hardtriplet_supportonly.py --only_test True --test_way 5 --test_shot 1
...
from src.loss import global_loss_support
...
from protonets.prototypical_loss import prototypical_loss_support as loss_fn
    ...
    for iter_n in range(args.iteration): 
        ...
        for ep in range(args.epochs):
            ...
            for data, target in tqdm(train_loader):    
                ...
                protoloss, acc, support_idxs, query_idxs = loss_fn(feat, target=target, n_support=args.train_shot)
                protoloss = protoloss.to(device)     
                
                triloss, _, _, _, _, _ = global_loss_support(tri_loss, feat, target, support_idxs, query_idxs, normalize_feature=args.normalize_feature, hard_mining=args.hard_mining)   
...
def test(args, model, device):
    ...
    for epoch in range(5):
        ...
        for batch in test_iter:
            ...
            _, acc, _, _ = loss_fn(model_output, target=y, n_support=args.test_shot)
...
def main(args):
    ...
    if args.exp_dir == '':
        exp_dir = 'exp_proto_hardtriplet_supportonly'
