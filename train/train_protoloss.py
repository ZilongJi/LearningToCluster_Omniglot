"""
python ./train/train_protoloss.py --trainsize -1 --base_lr 0.005 --rho 0.5e-3 --batch-size 60 --ims_per_id 6 --epochs 50 --exp_decay_at_epoch 25

testing in 5-way 1-shot mode: 
    python ./train/train_protoloss.py --only_test True --test_way 5 --test_shot 1
...
def train(args, model, device, optimizer, exp_dir):
    ...
    for iter_n in range(args.iteration): 
        ...
        for ep in range(args.epochs):
            ...
            loss_log = (', acc {:.2%}, protoloss {:.4f}'.format(protoacc_meter.avg, protoloss_meter.avg))             
...
def main(args):
    ...
    train(args, model, device, optimizer, exp_dir) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ims_per_id', type=int, default=6, 
                        help="ims_per_id must >1 when using triplet loss or prototype loss, value=train shot + train query")
